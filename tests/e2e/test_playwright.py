...
#          The live_server fixture (tests/e2e/conftest.py) auto-starts uvicorn on port 8001.
import re
import uuid
import pytest
...
def unique_user():
    s = uuid.uuid4().hex[:8]
    return {"username": f"e2e_{s}", "email": f"e2e_{s}@test.com",
            "password": "E2ePass1!", "first_name": "E2E", "last_name": "Tester"}
...
def add_calc(page: Page, op: str, values: str):
    """Select operation, fill multi-value input, click Calculate."""
...
def test_calculator_icon_visible_on_home(page: Page):
    page.goto(BASE_URL)
    # SVG rect is part of the calculator icon inside .hero-icon
...
def test_register_password_mismatch_shows_error(page: Page):
    page.goto(f"{BASE_URL}/register")
    page.fill("#first_name", "Bad"); page.fill("#last_name", "User")
    page.fill("#email", f"bad_{uuid.uuid4().hex[:6]}@test.com")
    page.fill("#username", f"bad_{uuid.uuid4().hex[:6]}")
    page.fill("#password", "E2ePass1!"); page.fill("#confirm_password", "Different9!")
    page.click("button[onclick='handleRegister()']")
...
def test_add_addition_two_values(page: Page):
    user = unique_user(); register_and_login(page, user)
...
def test_browse_shows_calculations_list(page: Page):
    user = unique_user(); register_and_login(page, user)
    add_calc(page, "addition", "1, 2"); page.wait_for_selector("#calc-success:visible")
    add_calc(page, "subtraction", "10, 3"); page.wait_for_selector("#calc-success:visible")
    expect(page.locator(".calculation-row")).to_have_count(2, timeout=5000)

@pytest.mark.e2e
def test_browse_shows_badges(page: Page):
    user = unique_user(); register_and_login(page, user)
    add_calc(page, "multiplication", "3, 4"); page.wait_for_selector("#calc-success:visible")
...
def test_read_view_page_shows_details(page: Page):
    user = unique_user(); register_and_login(page, user)
    add_calc(page, "addition", "3, 7"); page.wait_for_selector("#calc-success:visible")
...
def test_read_shows_all_inputs(page: Page):
    user = unique_user(); register_and_login(page, user)
    add_calc(page, "addition", "10, 20, 30"); page.wait_for_selector("#calc-success:visible")
    page.locator(".view-btn").first.click()
    expect(page).to_have_url(re.compile(r"/view/"))
    # All three input values should appear in the detail table
    for val in ["10", "20", "30"]:
...
def test_edit_updates_result(page: Page):
    user = unique_user(); register_and_login(page, user)
    add_calc(page, "addition", "3, 3"); page.wait_for_selector("#calc-success:visible")
...
def test_edit_form_prefilled(page: Page):
    user = unique_user(); register_and_login(page, user)
    add_calc(page, "multiplication", "4, 5"); page.wait_for_selector("#calc-success:visible")
    page.locator(".edit-btn").first.click()
    expect(page).to_have_url(re.compile(r"/edit/"))
...
def test_edit_division_by_zero_shows_error(page: Page):
    user = unique_user(); register_and_login(page, user)
    add_calc(page, "division", "20, 4"); page.wait_for_selector("#calc-success:visible")
    page.locator(".edit-btn").first.click()
...
def test_delete_removes_row(page: Page):
    user = unique_user(); register_and_login(page, user)
    add_calc(page, "addition", "10, 10"); page.wait_for_selector("#calc-success:visible")
    add_calc(page, "addition", "20, 20"); page.wait_for_selector("#calc-success:visible")
...
def test_delete_does_not_remove_other_rows(page: Page):
    user = unique_user(); register_and_login(page, user)
    add_calc(page, "addition", "1, 1");  page.wait_for_selector("#calc-success:visible")
    add_calc(page, "addition", "99, 1"); page.wait_for_selector("#calc-success:visible")
    page.wait_for_selector(".calculation-row")

    page.on("dialog", lambda d: d.accept())
    page.locator(".delete-btn").first.click()
    page.wait_for_timeout(1500)
    # The second calc (99+1=100) should still be there
