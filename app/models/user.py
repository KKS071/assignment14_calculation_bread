...
# Purpose: User SQLAlchemy model with auth helpers (register, authenticate, tokens).
import uuid
...
class User(Base):
    ...
    def register(cls, db, user_data: dict):
        """Register a new user. Raises ValueError on invalid data or duplicates."""
    ...
    def authenticate(cls, db, username_or_email: str, password: str):
        """Authenticate by username/email + password. Returns token dict or None."""
    ...
    def verify_token(cls, token: str):
        """Decode an access token, returning UUID on success or None on failure."""
